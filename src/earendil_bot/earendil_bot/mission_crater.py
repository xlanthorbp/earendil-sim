#!/usr/bin/env python3
"""Navigate the rover to the crater coordinate using Nav2.

Run this manually after the rover has exited the starting enclosure.

Usage:
    ros2 run earendil_bot mission_crater --ros-args -p use_sim_time:=true
"""
...
# ─────────────────────────────────────────────
TARGET_LAT = 39.924915  # Map y ≈ +15  (Q1)
TARGET_LON = 32.836722  # Map x ≈ +20  (Q1)
...
def main():
    ...
    service_node = rclpy.create_node('mission_crater_services')
    ...
    navigator.get_logger().info(
        f'=== Navigating to crater: GPS({TARGET_LAT:.6f}, {TARGET_LON:.6f}) → '
    ...
    if result == TaskResult.SUCCEEDED:
        navigator.get_logger().info('✅ Crater reached!')
    ...
    navigator.get_logger().info('mission_crater shutting down.')
    rclpy.shutdown()
    import sys
    sys.exit(0)
