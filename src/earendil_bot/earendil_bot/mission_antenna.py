#!/usr/bin/env python3
"""Navigate the rover to the antenna coordinate using Nav2.

Run this manually after the rover has exited the starting enclosure.

Usage:
    ros2 run earendil_bot mission_antenna --ros-args -p use_sim_time:=true
"""
...
# ─────────────────────────────────────────────
#  TARGET GPS COORDINATE  (corresponds to map x=-25, y=-15)
# ─────────────────────────────────────────────
TARGET_LAT = 39.925185
TARGET_LON = 32.837249
...
def main():
    ...
    service_node = rclpy.create_node('mission_antenna_services')
    ...
    navigator.get_logger().info(
        f'=== Navigating to antenna: GPS({TARGET_LAT:.6f}, {TARGET_LON:.6f}) → '
    ...
    if result == TaskResult.SUCCEEDED:
        navigator.get_logger().info('✅ Antenna reached!')
    ...
    navigator.get_logger().info('mission_antenna shutting down.')
    rclpy.shutdown()
    import sys
    sys.exit(0)
